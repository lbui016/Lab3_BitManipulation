...
int main(void) {
	...
	tmpAhigh = tmpAhigh >> 4; //shift over 4 to get PA6..PA4
	if (tmpAhigh == 0x03) { //PC7 lights if a key is in ignition (PA4 = 1), driver is seated (PA5 = 1), and belt not fastened (PA6 = 0)
		seatbelt = seatbelt | 0x80;
	}

	tmpC = seatbelt | low | tmpC;
	PORTC = tmpC;
    }
    return 1;
}
